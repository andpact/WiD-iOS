...
/*
 ...
 DatePieChartView -> Day 뷰에 들어가는 가장 큰 파이 차트
 */
...
/**
 DiaryPieChartView -> DayDiary에 들어가는 중간 크기 뷰
 */
...
//                    .position(x: geo.size.width / 2, y: geo.size.width / 3)
    ...
    //                    .position(x: geo.size.width / 2, y: geo.size.width / 2)
                    ...
                    Text("%")
    //                    .font(.system(size: geo.size.width / 5, weight: .heavy))
                        .font(.system(size: geo.size.width / 12, weight: .black))
    //                    .position(x: geo.size.width / 2, y: geo.size.width / 2)
                }
...
/**
 PeriodPieChartView -> Week, Month, Search에 들어가는 가장 작은 파이 차트
 */
...
//                    .fontWeight(pieChartDataArray.count == 1 ? nil : .bold)
                    .foregroundColor(pieChartDataArray.count == 1 ? Color("DarkGray") : Color("Black-White"))
            }
        }
        .aspectRatio(contentMode: .fit)
    }
...
/**
 SearchPieChartView -> Search에 들어가는 뷰, 중앙에 퍼센트만 표시함.
 */
struct SearchPieChartView: View {
    private let date: Date
    private let wiDList: [WiD]
...
//                    .foregroundColor(pieChartDataArray.count == 1 ? Color("DarkGray") : Color("Black-White"))
                ...
                HStack(spacing: 0) {
                    Text("\(totalDurationPercentage)")
    //                    .font(.system(size: geo.size.width / 5, weight: .heavy))
                        .font(.system(size: geo.size.width / 4, weight: .black))
                        .foregroundColor(pieChartDataArray.count == 1 ? Color("DarkGray") : Color("Black-White"))
                    
                    Text("%")
    //                    .font(.system(size: geo.size.width / 5, weight: .heavy))
                        .font(.system(size: geo.size.width / 8, weight: .black))
                        .foregroundColor(pieChartDataArray.count == 1 ? Color("DarkGray") : Color("Black-White"))
                }
            }
        }
...
//                .environment(\.colorScheme, .dark)
            ...
            DiaryPieChartView(wiDList: [])
                .environment(\.colorScheme, .light)

            DiaryPieChartView(wiDList: [])
                .environment(\.colorScheme, .dark)
            
//            SearchPieChartView(date: Date(), wiDList: [])
//                .environment(\.colorScheme, .light)
//
//            SearchPieChartView(date: Date(), wiDList: [])
//                .environment(\.colorScheme, .dark)
